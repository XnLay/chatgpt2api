...
本文说明如何配合 `docker-compose.warp.yml` 中的 WARP、Privoxy、FlareSolverr，解决上游请求遇到 Cloudflare 拦截的问题。

## 工作方式

应用不会每个请求都调用 FlareSolverr。当前逻辑是：

1. 上游请求先按配置走代理链路。
2. 如果响应被识别为 Cloudflare challenge 或 403/503 拦截，后端调用 FlareSolverr。
3. FlareSolverr 打开目标地址并返回 `cf_clearance` cookie 和 User-Agent。
4. 后端缓存 clearance，并在后续上游请求中复用。
...
保存并测试通过后，继续使用图片、搜索或 OpenAI 兼容接口请求。如果 clearance 刷新失败，通常表示当前 IP、WARP 出口或 FlareSolverr 浏览器环境仍被拦截。
